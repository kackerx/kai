...
.PHONY: start
start:
	@go run -ldflags "-X main.VERSION=$(RELEASE_TAG)" ./cmd/main.go web -c ./configs/config.toml -m ./configs/model.conf --menu ./configs/menu.yaml
...
swagger:
	...
	@swag init --generalInfo ./cmd/main.go --output ./app/interfaces/api/swagger
...
.PHONY: docker-push
docker-push:
	time docker build --file ./Dockerfile --no-cache --tag kackerx/kai .
	docker login
	docker push kackerx/kai
